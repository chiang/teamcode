...
import stageJob from './pipeline-stage-job';
...
export default {
  ...
  methods: {
    fetchBuilds(e) {
      ...
      axios.get(this.stage.path)
        .then((response) => {
            this.builds = response.data.jobs;
        });
  ...
  computed: {
    ...
    icon(){
      ...
      return svgsDictionary[this.stage.status.icon];
    },
  },
  template: `
    <div>
      ...
      <ul class="dropdown-menu mini-pipeline-graph-dropdown-menu js-builds-dropdown-container">
        ...
        <div
          ...
          <div v-if="builds == null" v-html="spinner"></div>
          <job v-else v-for="build in builds" :job="build"/>
        </div>
      </ul>
    </div>
  `,
